...
class Parameter(relax.Var):
    ...
    def __init__(
        ...
        dtype="float32",
        name="param",
        default_value: tvm.nd.NDArray = None,
    ):
        if not isinstance(shape, (list, tuple)):
...
class Module:
    ...
    def param_default_values(self) -> List[tvm.nd.NDArray]:
        return [p.default_value for p in self.parameters() if p.default_value is not None]

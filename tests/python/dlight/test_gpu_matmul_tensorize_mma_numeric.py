...
def do_numeric_test(
    ...
    with target:
        after_mod = dl.ApplyDefaultSchedule(dl.gpu.MatmulTensorizationMMA())(before_mod)
...
def test_nt_matmul_mixed_precision():
    @T.prim_func
    ...
    do_numeric_test(before, inputs, lambda x, y: np.matmul(x, y.T), Target("cuda"), tvm.cuda())


def test_nn_matmul_mixed_precision():
    @T.prim_func
    def before(p_A: T.handle, p_B: T.handle, p_O: T.handle):
        b = T.int64()
        A = T.match_buffer(p_A, (b, T.int64(256), T.int64(256)), "float16")
    ...
    do_numeric_test(before, inputs, lambda x, y: np.matmul(x, y), Target("cuda"), tvm.cuda())


def test_tn_matmul_mixed_precision():
    @T.prim_func
    def before(p_A: T.handle, p_B: T.handle, p_O: T.handle):
        b = T.int64()
        A = T.match_buffer(p_A, (b, T.int64(256), T.int64(256)), "float16")
    ...
    do_numeric_test(
        before, inputs, lambda x, y: np.matmul(x.transpose(0, 2, 1), y), Target("cuda"), tvm.cuda()
    )


def test_tt_matmul_mixed_precision():
    @T.prim_func
    def before(p_A: T.handle, p_B: T.handle, p_O: T.handle):
        b = T.int64()
        A = T.match_buffer(p_A, (b, T.int64(256), T.int64(256)), "float16")
    ...
    do_numeric_test(
        before,
        inputs,
        lambda x, y: np.matmul(x.transpose(0, 2, 1), y.T),
        Target("cuda"),
        tvm.cuda(),
    )

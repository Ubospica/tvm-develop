...
class PermutedLayoutInjector : private IRMutatorWithAnalyzer {
 ...
 private:
  explicit PermutedLayoutInjector(PrimFunc func, Analyzer* analyzer)
      : IRMutatorWithAnalyzer(analyzer), func_(std::move(func)) {}
  ...
  Stmt VisitStmt_(const BlockNode* op) final {
    ...
    if (!CheckAnnotation(op->annotations.at("permuted_layout"))) {
      return IRMutatorWithAnalyzer::VisitStmt_(op);
    }
  ...
  PrimExpr HandleAccessPtrAndOffset(PrimExpr access_ptr, Optional<PrimExpr> offset = NullOpt) {
    ...
    auto buffer = LookupBufferFromData(Downcast<Var>(access_ptr_call->args[1]), func_);
    CHECK(buffer) << "The buffer corresponding to data Var " << access_ptr_call->args[1]
                  << " is not found";
    int buffer_row_size = CheckAndGetBufferRowSize(buffer.value());
